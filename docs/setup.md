...
- If you'll be editing SQL queries or the database schema: the
  [golang-migrate CLI](https://github.com/golang-migrate/migrate/releases)
  and [sqlc CLI](https://docs.sqlc.dev/en/latest/overview/install.html) -
  ```bash
...
# ThunderID - one-time setup, see the ThunderID Setup guide, then come back here

# Backend - runs migrations automatically on startup
cd backend
go mod download
cp .env.example .env   # fill in DB + identity-provider values, plus SMTP_* (see below)
